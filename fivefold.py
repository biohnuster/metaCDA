...
if __name__ == '__main__':
    ...
    A = np.loadtxt('dataliu/associationMatrix_585_88.csv', delimiter=',')
    circSimi = np.loadtxt('dataliu/Integrated_sqe_fun_circRNA_similarity_585.csv', delimiter=',')
    disSimi = np.loadtxt('dataliu/Integrated_gip_DO_disease_similarity_88.csv', delimiter=',')
    ...
    for i in range(0, len(positive_index_list), positive_split):
        ...
        a = 1
        model = metaCDA(585, 88, 128, 8, 0.1, 0.3, 2778, 1, mat_c).to(device)
        # 声明参数优化器
