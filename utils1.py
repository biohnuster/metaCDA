...
def constructHNet(drug_dis_matrix, drug_matrix, dis_matrix, u):
    mat1 = np.hstack((u * drug_matrix, drug_dis_matrix))
    mat2 = np.hstack((drug_dis_matrix.T, u * dis_matrix))
    out = np.vstack((mat1, mat2))
...
def normalize(matrix):
    matrix = matrix + torch.eye(matrix.shape[0])
    deg = torch.sum(matrix, dim=1)
    deg[deg <= 1e-10] = 1
    deg_inv = deg.pow(-0.5)
    deg_inv = deg_inv * torch.eye(matrix.shape[0]).type(torch.FloatTensor)
    matrix_right = th.mm(deg_inv, matrix)
    matrix_normal = th.mm(matrix_right, deg_inv)
